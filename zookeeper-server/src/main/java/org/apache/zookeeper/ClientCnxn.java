...
public class ClientCnxn {
    ...
    public ClientCnxn(String chrootPath, HostProvider hostProvider, int sessionTimeout, ZooKeeper zooKeeper,
        ...
        sendThread = new SendThread(clientCnxnSocket);  //基于socket给server发送数据
        eventThread = new EventThread();  //处理Server发送过来的事件
    ...
    class SendThread extends ZooKeeperThread {
        ...
        void primeConnection() throws IOException {
            ...
            if (!clientConfig.getBoolean(ZKClientConfig.DISABLE_AUTO_WATCH_RESET)) {
                ...
                List<String> dataWatches = zooKeeper.getDataWatches(); //监听znode数据的变化
                List<String> existWatches = zooKeeper.getExistWatches();//监听znode是否存在
                List<String> childWatches = zooKeeper.getChildWatches();//监听znode下的子node
        ...
        public void run() {
            ...
            while (state.isAlive()) {
                try {
                    ...
                    //第一次进入，则outgoingQueue中会有一个ConnectRequest请求
    ...
    public Packet queuePacket(RequestHeader h, ReplyHeader r, Record request,
        ...
        synchronized (outgoingQueue) {
            //查看这个packet的状态，这个类ClientCnxn是与客户建立连接的
        ...
        //向channel中写入一个空字节，唤醒SendThread中的selector
        //唤醒select函数
