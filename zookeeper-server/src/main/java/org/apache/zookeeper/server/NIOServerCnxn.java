...
public class NIOServerCnxn extends ServerCnxn {
    ...
    private void readPayload() throws IOException, InterruptedException {
        ...
        //如果有remaining，则说明没有读满，由于incomingBuffer和请求包的大小一样，如果remaining读满，则说明这个请求完全读了
        if (incomingBuffer.remaining() == 0) { // have we read length bytes?
            //统计
            packetReceived();
            //切换读模式
            ...
            //initialized没有，则说明需要接收客户端的ConnectRequest
            if (!initialized) {
                //在服务端维护Session
                readConnectRequest();
            } else {
            ...
            if (k.isReadable()) {
                ...
                if (incomingBuffer.remaining() == 0) { //保证读取了完整的4个字节 或者读完了一个请求
                    ...
                    if (incomingBuffer == lenBuffer) { // start of next request
                        incomingBuffer.flip();
                        isPayload = readLength(k); //获取当前的长度，创建一个byteBuffer 这里incomingBuffer就是长度等于数据包长度的Buffer了
