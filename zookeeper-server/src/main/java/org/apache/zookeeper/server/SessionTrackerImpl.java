...
public class SessionTrackerImpl extends ZooKeeperCriticalThread implements
    ...
    // 过期队列。
    // 用于维护会话的过期，并且使用bucket来维护会话，每一个bucket对应一个某时间范围内过期的会话。
    ...
    // 高八位:最多部署127个机器
    // 最后16位:最后16位说明一毫秒能产生16位的id，也就是65536个
    public static long initializeNextSession(long id) {
        long nextSid;

        //获取当前系统的时间戳 左移24和无符号右移(不会移动最高位)都是为了防止负数
    ...
    synchronized public boolean touchSession(long sessionId, int timeout) {
        ...
        //这个session已经都读写或者心跳了，给他续命，更新过期map的key
