...
public class FileTxnLog implements TxnLog, Closeable {
    ...
    public synchronized void rollLog() throws IOException {
        if (logStream != null) {
            ...
            //设置为null，写事务日志的时候会判断logStream是不是null，是null则产生新文件
            this.logStream = null;
            oa = null;
        }
    }
        ...
        throws IOException
    {
        ...
        //如果WAL的IO流为空（重启后这里一定是空）
        if (logStream==null) {
           if(LOG.isInfoEnabled()){
                LOG.info("Creating new log file: " + Util.makeLogName(hdr.getZxid()));
           }
           //logDir是parentDir  log文件名是log.(zxid)
            //zxid 是一个 long 类型的字段，前 32 位代表朝代，我们这里称为 epoch，后 32 位称为事务次数，也就是写请求次数
            //将 zxid 作为文件后缀可以清楚地看出当前运行时 ZooKeeper 的 leader 周期以及写了多少次事务，某种意义上也保证了日志文件是有序的，因为 zxid 是有序的
            //例如 log.64fe
           logFileWrite = new File(logDir, Util.makeLogName(hdr.getZxid()));
           fos = new FileOutputStream(logFileWrite);
           //文件的IO流，层层包装，最后包装为OutputArchive
           logStream=new BufferedOutputStream(fos);
           oa = BinaryOutputArchive.getArchive(logStream);
           //写一个文件头，标识一下这个log文件的信息
            //文件头包含
            //  1、4字节魔数 ZKLG
            //  2、2字节版本号 2
            //  3、8字节 dbld
        ...
        //到现在准备工作已经完成  log.zxid 文件+文件头+填充 64MB 数据

        //一条log日志的组成为 事务请求头、事务请求体、(签名)、校验和
        //开始写 1、将hdr和txn处理为字节数组,这里将序列化的操作组合在TxnHeader和Record
        byte[] buf = Util.marshallTxnEntry(hdr, txn);
        if (buf == null || buf.length == 0) {
            throw new IOException("Faulty serialization for header " +
                    "and txn");
        }
        //2、校验和 ，基于事务请求头、事务请求体、(签名)这三者来生成
        Checksum crc = makeChecksumAlgorithm();
        crc.update(buf, 0, buf.length);
        //写入校验和
        oa.writeLong(crc.getValue(), "txnEntryCRC");
        //写入文件(对于操作系统是写入了Buffer缓冲区，需要再commit SyncRequestProcessor 的时候fsync)
