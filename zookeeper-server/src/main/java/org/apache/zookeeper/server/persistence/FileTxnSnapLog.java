...
public class FileTxnSnapLog {
    ...
    public long fastForwardFromEdits(DataTree dt, Map<Long, Integer> sessions,
                                     PlayBackListener listener) throws IOException {
        //这里的dt是从快照文件恢复到内存中的，所以直接读取dt中的zxid就可以了
        //而 dt.lastProcessedZxid 是在恢复快照日志的时候赋值的，因此只需要找dt.lastProcessedZxid + 1 的事务日志就好
        ...
        try {
            while (true) {
                ...
                try {
                    //进行数据恢复
                    //扫描事务日志文件夹下的全部文件，拿到文件后缀大于 zxid+1 的
                    //拿到后按 zxid 的升序顺序（从最小的开始）逐个反序列化恢复到内存
