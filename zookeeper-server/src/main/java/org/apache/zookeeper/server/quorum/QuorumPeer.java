                ...
                switch (getPeerState()) {
                    case LOOKING:
                        ...
                        if (Boolean.getBoolean("readonlymode.enabled")) {
                            ...
                            //不特殊配置的话一般是走这里，不是ReadOnlyServer
                            try {
                                reconfigFlagClear();
                                if (shuttingDownLE) {
                                    shuttingDownLE = false;
                    ...
                    case OBSERVING:
                        try {
                            ...
                            //里面有个死循环，当前线程去处理、阻塞，所以最外层的whileTrue没有问题，这个线程没有被浪费、空转
                            //建立连接 同步数据
                    ...
                    case FOLLOWING:
                        try {
                            ...
                            //建立连接 同步数据
                            //非 Leader 节点也具有持久化功能，宕机重启后会先从磁盘上恢复数据到内存，
                            // 丢失的是宕机重启这期间 Leader 节点变化的数据，因此我们只需要同步这期间差异化的数据即可，
                            // 我们称这种数据同步方式为直接差异化同步（DIFF）
